...
export default function LeadsTable({ initialLeads, initialCount }: LeadsTableProps) {
    const { refreshUser } = useGlobal();
    const [data, setData] = useState<LeadRow[]>(initialLeads);
    ...
    const fetchLeads = useCallback(async () => {
        ...
        try {
            ...
            if (qualityTier) params.set('qualityTier', qualityTier);
            if (scoreMin) params.set('scoreMin', scoreMin);
            if (rentaTotalMin) params.set('rentaTotalMin', rentaTotalMin);
            if (maxDividendoMin) params.set('maxDividendoMin', maxDividendoMin);
            if (bancarizado !== 'all') params.set('bancarizado', bancarizado === 'si' ? 'true' : 'false');
            if (ahorros !== 'all') params.set('ahorros', ahorros === 'si' ? 'true' : 'false');
    ...
    useEffect(() => {
        // Skip the initial fetch since we have server-provided data
        // Only re-fetch when filters or page change from their defaults
        const isInitialState = !qualityTier && !scoreMin && !rentaTotalMin && !maxDividendoMin
            && bancarizado === 'all' && ahorros === 'all' && !meetingDate && page === 1;
        if (!isInitialState) {
            fetchLeads();
        }
    }, [fetchLeads, qualityTier, scoreMin, rentaTotalMin, maxDividendoMin, bancarizado, ahorros, meetingDate, page]);

    const handleReserve = async (leadId: string | null) => {
        if (!leadId) return;
        setActionLoading(leadId);
        try {
            const res = await fetch(`/api/leads/${leadId}/reserve`, { method: 'POST' });
