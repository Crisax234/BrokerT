...
export default function StockClient({ initialProjects, initialUnitCounts }: StockClientProps) {
    return (
        <Suspense fallback={
            ...
            <StockPageContent initialProjects={initialProjects} initialUnitCounts={initialUnitCounts} />
        </Suspense>
    );
}

function StockPageContent({ initialProjects, initialUnitCounts }: StockClientProps) {
    const searchParams = useSearchParams();
    const router = useRouter();
    const leadId = searchParams.get('leadId');

    // Projects + counts (Phase A) — initialized from server props
    const [projects] = useState<ProjectRow[]>(initialProjects);
    const [unitCounts] = useState<Record<string, number>>(initialUnitCounts);
    ...
    // Fetch units when project selected — via API route
    const fetchUnits = useCallback(async () => {
        ...
        try {
            ...
            if (statusIn) params.set('statusIn', statusIn.join(','));
            params.set('page', String(page));
            params.set('pageSize', String(pageSize));

            const res = await fetch(`/api/units?${params.toString()}`);
            if (!res.ok) {
                console.error('Error fetching units:', res.statusText);
                return;
            }
            const json = await res.json();
            setUnits((json.data.units as UnitRow[]) ?? []);
