...
export default function EscenarioPage() {
    ...
    // Support both ?units=A,B,C (multi) and ?unit=A (single, backwards compat)
    const initialUnits: string[] = useMemo(() => {
        const unitsParam = searchParams.get('units');
        if (unitsParam) return unitsParam.split(',').map(s => s.trim()).filter(Boolean).slice(0, 5);
    ...
    const [unitLookup, setUnitLookup] = useState<UnitLookup[]>([]);
    ...
    useEffect(() => {
        async function load() {
            try {
                ...
                const [projectRes, ufRes, unitsRes] = await Promise.all([
                    client.getProjectById(projectId),
                    client.getLatestUFValue(),
                    client.getProjectUnitNumbers(projectId),
                ]);

                if (projectRes.data) setProject(projectRes.data as ProjectRow);
                if (ufRes.data) {
                    setUfValue(ufRes.data.value);
                }
                if (unitsRes.data) setUnitLookup(unitsRes.data as UnitLookup[]);
            } catch (err) {
                console.error('Error loading escenario data:', err);
            } finally {
                setLoading(false);
            }
        }
    ...
    // ── Fetch initial units when lookup is ready ─────
    useEffect(() => {
